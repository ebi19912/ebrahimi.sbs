    ...
    <div className="fixed bottom-5 right-5 sm:bottom-8 sm:right-8 z-50 font-sans">
      ...
      {!isOpen && (
        <button 
          ...
          className="bg-gradient-to-tr from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center shadow-xl hover:shadow-2xl hover:scale-105 active:scale-95 transition-all duration-200"
      ...
      {/* Chat Window Modal (Responsive for all mobile screens) */}
      {isOpen && (
        <div className="fixed sm:absolute bottom-3 right-3 left-3 sm:left-auto sm:bottom-0 sm:right-0 w-auto sm:w-[380px] h-[78vh] sm:h-[520px] max-h-[90vh] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-200/80 z-50">
          {/* Header */}
          ...
          <div className="flex-1 p-4 bg-slate-50 overflow-y-auto flex flex-col gap-3.5">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div 
                  className={`max-w-[88%] p-3.5 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-sm break-words ${
                    msg.role === 'user' 
            ...
            {isLoading && (
              <div className="flex flex-col items-start">
                <div className="p-3.5 rounded-2xl text-sm shadow-sm bg-white text-slate-800 rounded-bl-none border border-slate-200/60">
                  <div className="text-[11px] font-bold text-purple-600 mb-1 flex items-center gap-1">
                    <BiBot /> AI Assistant
                  </div>
